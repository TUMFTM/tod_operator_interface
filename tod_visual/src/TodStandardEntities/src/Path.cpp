...
Entity Path::create(std::shared_ptr<Scene> scene, const std::string &name,
    ...
    transform.setTranslation(glm::vec3(0.0f, 0.0f, 0.025f)); // move up to put on top of videos
    return laneEntity;
}

void Path::onPathReceived(const nav_msgs::PathConstPtr& msg, Entity &pathEntity) {
    ...
    for (int i = 0; i != std::min(15, (int) msg->poses.size()); ++i) {
        auto& pos = msg->poses.at(i).pose.position;
